...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    {   
        if(count(&q) >= 10)
        {
            ...
            memset(temp_img_path, 0, sizeof(temp_img_path));
            char *a = dequeue(&q);
            strcpy(img_path, a);
            free(a);
            // printf("strcpy(img_path, dequeue(&q)); %s\n", img_path);
            // printf("%s %ld\n", dequeue(&q), strlen(dequeue(&q)));
            strncpy(temp_img_path, img_path, strlen(img_path));
            image im = load_image_color(img_path,0,0);
            image sized = letterbox_image(im, net->w, net->h);
            layer l = net->layers[net->n-1];
            // !pop image by img_path

            // 3. detections
            float *X = sized.data;
            time=what_time_is_it_now();
            network_predict(net, X);
            //printf("%s: Predicted in %f seconds.\n", temp, what_time_is_it_now()-time);
            ...
            // 4. find place
            char* temp = strtok(temp_img_path, ".");
            char* temp1 = strtok(temp, "/");
            temp1 = strtok(NULL," ");
            char* place = strtok(temp1, "_");
            place = strtok(NULL," ");
            // !find place
        
            //5. draw box, create curl command 
            draw_detections(im, dets, nboxes, thresh, names, alphabet, l.classes, place); //curl command
            free_detections(dets, nboxes);
            if(outfile){
